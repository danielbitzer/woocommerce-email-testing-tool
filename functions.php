...
function run_email_script(){
	...
	switch ( $for_filter )
	{
		...
		case 'customer_new_account':
			$new_email->trigger( $user->ID );
			break;

		case 'customer_reset_password':
			$new_email->trigger( $user->user_login, 'test' );
			break;
...
function update_test_email_options() {
	...
	if( $_POST['wc_email_test_email']  ){
		...
		$result = update_option( "wc_email_test_email", $_POST['wc_email_test_email'] );
	...
	if( $_POST['wc_email_test_order_id']  ){
		...
		$result = update_option( "wc_email_test_order_id", $_POST['wc_email_test_order_id'] );
